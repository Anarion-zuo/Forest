...
FinalData::FinalData(const std::string &&dir) {
    using namespace std;
...
size_t FinalData::height() const {
    return _mat[0].size();
}

size_t FinalData::width() const {
    return _mat.size();
}

double FinalData::loc(size_t width, size_t height) const {
