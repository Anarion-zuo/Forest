...
L2Linear::L2Linear(const FinalData &X, const FinalData &negy) : _X(X), _y(negy), _dim(_X.width()) {
...
void L2Linear::gd_fit() {
    std::vector<double> w1(_dim + 1, 1.1), w2(_dim + 1, 1.1), sum(_dim + 1, 0);
    while (true) {
        double d;
        for (size_t row = 0; row < _X.height(); ++row){
            for (size_t i = 0; i < _dim; ++i){
                _inputs_X[i]->set_val(_X.loc(i, row));
                _weights[i]->set_val(w2[i]);
            }
            _weights[_dim]->set_val(w2[_dim]);
            _inputs_negy->set_val(-_y.loc(0, row));
//            w1 = w2;
            for (size_t i = 0; i <= _dim; ++i){
                _node->set_child(1, _gradterms[i]);
                _node->compute();
